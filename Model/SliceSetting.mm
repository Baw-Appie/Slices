...
- (NSString *)getValueInDirectory:(NSString *)directory {
	...
	if (fullSettingFileName.length < 1) return NULL;
...
- (BOOL)setValueInDirectory:(NSString *)directory value:(NSString *)value {
	...
	if (value.length > 0) newFullSettingFileName = [self.prefix stringByAppendingString:value];
	else newFullSettingFileName = self.prefix;

	NSString *newFullSettingFilePath = [directory stringByAppendingPathComponent:newFullSettingFileName];

	// if the directory doesn't exist, create it
	NSFileManager *manager = [NSFileManager defaultManager];
	if (![manager fileExistsAtPath:directory]) [manager createDirectoryAtPath:directory withIntermediateDirectories:YES attributes:nil error:NULL];
	...
	if (fullSettingFileName.length > 0) {
		...
		if (value.length > 0) return [manager moveItemAtPath:fullSettingFilePath toPath:newFullSettingFilePath error:NULL];
		else return [manager removeItemAtPath:fullSettingFilePath error:NULL];
	}
